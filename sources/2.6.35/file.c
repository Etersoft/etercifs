...
cifs_posix_open_inode_helper(struct inode *inode, struct file *file,
			     struct cifsInodeInfo *pCifsInode, u16 netfid)
{
...
static inline int cifs_open_inode_helper(struct inode *inode,
	struct cifsTconInfo *pTcon, FILE_ALL_INFO *buf,
 ...
 *********************************************************************/
	...
	rc = cifs_open_inode_helper(inode, tcon, buf, full_path, xid);
	if (rc != 0)
		goto out;
		...
		if (pCifsInode) {
			...
			/* temporarily disable caching while we
			   go to server to get inode info */
				cifs_set_oplock_level(pCifsInode, 0);
				if (tcon->unix_ext)
...
int cifs_close(struct inode *inode, struct file *file)
{
	...
	if (list_empty(&(CIFS_I(inode)->openFileList))) {
		...
		cifs_set_oplock_level(CIFS_I(inode), 0);
	}
