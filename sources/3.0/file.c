 ...
 *********************************************************************/
	...
	if (tcon->ses->capabilities & CAP_NT_SMBS)
		rc = CIFSSMBOpen(xid, tcon, full_path, disposition,
			 desiredAccess, share_access, CREATE_NOT_DIR, pnetfid,
			 poplock, buf, cifs_sb->local_nls,
			 cifs_sb->mnt_cifs_flags & CIFS_MOUNT_MAP_SPECIAL_CHR);
	else
		rc = SMBLegacyOpen(xid, tcon, full_path, disposition,
			desiredAccess, CREATE_NOT_DIR, pnetfid,
			poplock, buf, cifs_sb->local_nls,
			cifs_sb->mnt_cifs_flags & CIFS_MOUNT_MAP_SPECIAL_CHR);
...
/* can not grab rename sem here because various ops, including
   ...
   and we can never tell if the caller already has the rename_sem */
	...
	share_access = cifs_get_share_flags(pCifsFile->f_flags);
	...
	rc = CIFSSMBOpen(xid, tcon, full_path, disposition, desiredAccess,
			 share_access, CREATE_NOT_DIR, &netfid, &oplock, NULL,
