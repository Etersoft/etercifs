...
CIFSSMBOpen(const int xid, struct cifsTconInfo *tcon,
	    ...
	    const int create_options, __u16 *netfid,
	    int *pOplock, FILE_ALL_INFO *pfile_info,
	    const struct nls_table *nls_codepage, int remap)
{
	int rc = -EACCES;
...
CIFSSMBPosixLock(const int xid, struct cifsTconInfo *tcon,
		const __u16 smb_file_id, const __u32 net_pid,
		const int get_flag, const __u64 len,
		...
		const bool waitFlag)
{
	...
	if (rc) {
		...
		if (parm_data->lock_type == __cpu_to_le16(CIFS_UNLCK))
